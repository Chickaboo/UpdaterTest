...
class Tournament:
    ...
    def get_standings(self, active_only=True):
        """Return a sorted list of players by score and tiebreakers. If active_only is False, include all players."""
        self.compute_tiebreakers()
        if active_only:
            player_list = self._get_active_players()
        else:
            player_list = list(self.players.values())
        # Sort by score, then tiebreakers, then rating, then name
        def cmp(p1, p2):
            if p1.score > p2.score: return -1
            if p1.score < p2.score: return 1
            for tb in self.tiebreak_order:
                v1 = p1.tiebreakers.get(tb, 0)
                v2 = p2.tiebreakers.get(tb, 0)
                if v1 > v2: return -1
                if v1 < v2: return 1
        ...
        import functools
        return sorted(player_list, key=functools.cmp_to_key(cmp))
